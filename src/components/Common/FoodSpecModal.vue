            ...
            <v-sheet class="text-center" height="100vh">
                ...
                >
                    <v-card-title>
                        ...
                        <span class="title green--text"><strike class="grey--text caption mr-1">&#8377;{{ item.gross_price }}</strike>&#8377;{{computedPrice()}}</span>
                    </v-card-title>
                    ...
                    <v-card-text v-if="item.varients.length>0">
                        <span class="text">Select Size</span>
                            <v-radio-group v-model="itemVarient">
                                <v-row>
                                <v-col class="mx-n1 my-n4 pt-7" 
                                cols="12" 
                               
                                v-for="varient in item.varients"  :key="varient.id">
                                      <v-radio
                                        class="my-1"
                    ...
                    <v-card-text v-if="item.extras.length>0">
                        <span class="text">Add Extras</span>
                             <v-row>
                            <v-col class="mx-n1 my-n4 pt-7"  cols="12"  v-for="extra in item.extras" :key="extra.id">
                                 <v-checkbox
                                class="my-n2"
                                :key="extra.id"
                                  v-model="extraAddons"
             ...
             if(this.itemVarient !=0) {
              ...
              if(this.item.extras.length>0 && this.extraAddons.length>0) {
                  this.extraAddons.map((addon)=> {
                      let addon_price = this.item.extras.find((addon_2)=> addon === addon_2.id).price;
                      if(addon_price)
                        extraTotal += addon_price
                  })
